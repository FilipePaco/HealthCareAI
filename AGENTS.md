...
## Stack
Python 3.11+ · **LangGraph** (orquestração) · **Gemini 2.5 Flash-Lite** via `init_chat_model`
...
- **Custo é observável.** Cada relatório mede tokens de LLM, embeddings do RAG e buscas Tavily
  (`governance/usage.py`) e estima o gasto, registrando modelo e tarifas aplicadas; exposto no
  relatório, na auditoria e em `GET /usage`. (P9/ADR-12/R10.4-R10.5)
- **Só agregados saem do banco.** Microdados nunca vão para a API, para o LLM nem para logs. (P4)
- **Auditoria em duas camadas.** A camada 1 (`src/governance/audit.py` → `audit_log` no Postgres) é
  obrigatória e é o registro de conformidade. A camada 2 (`src/governance/tracing.py` → Langfuse) é
  opcional (`TRACING_ENABLED`), some sem quebrar nada, e **só ela** pode importar `langfuse`.
  As duas são unidas pelo `report_id`. (P2/ADR-13)
...
- Ao rodar o `docker compose`, o Postgres fica exposto na porta **5433** do host (não 5432).
